...
class GraphGenerator(nn.Module):
    def __init__(self, num_joints=25, in_channels=3):
        ...
        self.mlp = nn.Sequential( nn.Linear(num_joints * in_channels, 128), nn.ReLU(), nn.Linear(128, num_joints * num_joints) )

    def forward(self, x): # x: [batch, joints, coords, frames]
        batch, joints, coords, frames = x.shape
        x_flat = x.mean(dim=3).view(batch, -1) # [batch, joints * coords]
        A = self.mlp(x_flat).view(batch, joints, joints)
        A = F.softmax(A, dim=-1) # 归一化边权重 return A
...
class Learner(nn.Module):
    def __init__(self, num_joints=25, in_channels=3, out_channels=128, num_classes=5):
        ...
        self.num_joints = num_joints # 图卷积层
        ...
        self.fc = nn.Linear(out_channels * num_joints, num_classes)# 分类头（N-way 分类）

        # self.vars = nn.ParameterList()  # 存储所有可优化的张量 (权重和偏置)
        #
        # self.vars_bn = nn.ParameterList()  # 存储批归一化层的running_mean和running_var

    def forward(self, x, A): # x: [batch, joints, coords, frames] # A: [batch, joints, joints]
         setsize, C, T, V = x.shape
         x = x.permute(0, 2, 1, 3) # [batch, c, v, t]
         x = F.relu(self.bn1(self.conv1(x))) # [batch, 64, joints, frames]
         x = x.permute(0, 2, 1, 3) # [batch, joints, 64, frames]
         x = torch.einsum('bij,bjcf->bicf', A, x) # 图卷积
         x = x.permute(0, 2, 1, 3) # [batch, 64, joints, frames]
         x = F.relu(self.bn2(self.conv2(x))) # [batch, out_channels, joints, frames]
         x = x.permute(0, 2, 1, 3) # [batch, joints, out_channels, frames]
         x = torch.einsum('bij,bjcf->bicf', A, x)
         x = x.permute(0, 2, 1, 3) # [batch, out_channels, joints, frames]
         x = x.mean(dim=3) # [batch, out_channels, joints]
         features = x.view(setsize, -1) # [batch, out_channels * joints]
         logits = self.fc(features) # [batch, num_classes]
         features = x.permute(0, 2, 1) # [batch, joints, out_channels]
