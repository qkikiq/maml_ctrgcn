...
class FewshotProcessor():
    """
        FewshotProcessor for Skeleton-based Action Recgnition
         用于基于骨骼的动作识别的处理类
    """
    ...
    #         self.global_step += 1
    #         for  step, (data_s, label_s, x_qry, y_qry) in enumerate(data,label):
    #             with torch.no_grad():
    #                 data_s = data_s.float().cuda(self.output_device)
    #                 label_s = label_s.float().cuda(self.output_device)
    #                 data_q = label.long().cuda(self.output_device)
...
if __name__ == '__main__':
    ...
    # 随机种子
    init_seed(arg.seed)
    processor = FewshotProcessor(arg)
    processor.start()
