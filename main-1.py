...
def get_parser():
    ...
    parser.add_argument('--train_sampler_args', type=dict, default={...})
...
class Processor():
    ...
    def load_data(self):
        Feeder = import_class(self.arg.feeder)  # 动态导入数据加载器类
        Sample = import_class(self.arg.sample)  #
        self.data_loader = dict()  # 初始化数据加载器字典
        # 如果是训练阶段，创建训练数据加载器
        if self.arg.phase == 'train':
            self.data_loader['train'] = torch.utils.data.DataLoader(
                dataset=Feeder(**self.arg.train_feeder_args),  # 实例化数据集类，传入参数
                ###todo 采样器
                sampler=Sample(
                    **self.arg.train_sampler_args),  # 不使用自定义采样器
                batch_size=self.arg.batch_size,  # 设置批次大小
                shuffle=False,  # 每个 epoch 都打乱数据
                num_workers=self.arg.num_worker,  # 设置数据加载的工作进程数
                drop_last=True,  # 丢弃最后一个不完整的批次
                worker_init_fn=init_seed)  # 为每个工作进程设置随机种子
        # 创建测试数据加载器
        self.data_loader['test'] = torch.utils.data.DataLoader(
            dataset=Feeder(**self.arg.test_feeder_args),  # 实例化测试数据集类
            batch_size=self.arg.test_batch_size,  # 设置测试批次大小
    ...
    def train(self, epoch, save_model=False):
        ...
        # 遍历训练数据加载器#todo
        for batch_idx, (data_s,data_q, label_s, label_q, index) in enumerate(process):
            ...
            loss_value.append(loss.item())  # 记录当前 batch 的损失值
            timer['model'] += self.split_time()  # 记录模型计算（前向+反向+更新）时间

            # === 计算准确率 ===
            value, predict_label = torch.max(output.data, 1)  # 找到预测概率最高的类别
            acc = torch.mean((predict_label == label.data).float())  # 计算准确率
            acc_value.append(acc.item())  # 记录当前 batch 的准确率值
            # 向 TensorBoard 写入当前 batch 的准确率和损失
            ...
            if hasattr(self, 'train_writer'):
                self.train_writer.add_scalar('lr', self.lr, self.global_step)  # 向 TensorBoard 写入学习率
            timer['statistics'] += self.split_time()  # 记录统计时间

        # statistics of time consumption and loss
        # 计算并打印本 epoch 的平均损失和准确率
        proportion = {
            k: '{:02d}%'.format(int(round(v * 100 / sum(timer.values())))) # 计算各部分时间占比
            for k, v in timer.items()
        }
        self.print_log(
            '\tMean training loss: {:.4f}.  Mean training acc: {:.2f}%.'.format(np.mean(loss_value), np.mean(acc_value)*100))
        self.print_log('\tTime consumption: [Data]{dataloader}, [Network]{model}'.format(**proportion)) # 打印时间消耗占比

        # 如果需要保存模型
        if save_model:
            # 创建一个新的 OrderedDict，移除 'module.' 前缀（如果存在，由 DataParallel 添加），并将权重移到 CPU
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        if wrong_file is not None:
            ...
            # 如果需要记录所有样本的预测结果，打开文件
        if result_file is not None:
            f_r = open(result_file, 'w')
        self.model.eval()  # 设置模型为评估模式（禁用 dropout, batchnorm 更新等）
        self.print_log('Eval epoch: {}'.format(epoch + 1))

        # 遍历指定的加载器名称（通常只有 'test'）
        for ln in loader_name:
            loss_value = []  # 记录损失值
            score_frag = []  # 记录模型输出的原始分数（logits）
            label_list = []  # 记录真实标签
            pred_list = []  # 记录预测标签
            step = 0
            # 使用 tqdm 创建进度条
            ...
            for batch_idx, (data, label, index) in enumerate(process):
                label_list.append(label)  # 记录真实标签 (CPU numpy array)
                with torch.no_grad():
                    data = data.float().cuda(self.output_device)
                    label = label.long().cuda(self.output_device)
                    output = self.model(data)  # 模型进行前向计算
                    loss = self.loss(output, label)  # 计算损失
                    score_frag.append(output.data.cpu().numpy())   # 将模型输出分数移动到 CPU 并转换为 numpy 数组，然后添加到列表中
                    loss_value.append(loss.data.item())  # 记录当前 batch 的损失值

                    _, predict_label = torch.max(output.data, 1)  # 找到预测概率最高的类别
                    pred_list.append(predict_label.data.cpu().numpy()) # 将预测标签移动到 CPU 并转换为 numpy 数组，然后添加到列表中
                    step += 1

                # 如果需要记录错误/所有结果文件
                if wrong_file is not None or result_file is not None:
                    predict = list(predict_label.cpu().numpy()) # 预测标签
                    true = list(label.data.cpu().numpy()) # 真实标签
                    # 遍历当前批次的每个样本
            ...
            if 'ucla' in self.arg.feeder:
                ...
                # 使用数据集对象内置的方法计算 Top-1 准确率
            accuracy = self.data_loader[ln].dataset.top_k(score, 1)
            if accuracy > self.best_acc:
                self.best_acc = accuracy   # 更新最佳准确率
                self.best_acc_epoch = epoch + 1 # 更新最佳准确率对应的轮次
